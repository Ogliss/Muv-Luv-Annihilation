		...
		*/
		...
		private static bool CellHasBlockingThings(IntVec3 cell, Map map)
		{
			...
			for (int i = 0; i < thingList.Count; i++)
			{
				...
				if (thingList[i].def.category == ThingCategory.Building && thingList[i].def.passability == Traversability.Impassable && GenSpawn.SpawningWipes(RimWorld.ThingDefOf.Hive, thingList[i].def))
				{
					return true;
				}
			}
