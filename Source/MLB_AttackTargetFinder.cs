    ...
    public static class MLB_AttackTargetFinder
    {
        ...
        public static IAttackTarget BestAttackTarget(IAttackTargetSearcher searcher, TargetScanFlags flags, Predicate<Thing> validator = null, float minDist = 0f, float maxDist = 9999f, IntVec3 locus = default(IntVec3), float maxTravelRadiusFromLocus = 3.40282347E+38f, bool canBash = false)
        {
            ...
            if (MLB_AttackTargetFinder.HasRangedAttack(searcher))
            {
                ...
                {
                    MLB_AttackTargetFinder.tmpTargets.Add(iTarget);
                }

                if ((byte)(flags & TargetScanFlags.NeedReachable) != 0)
                {
                    Predicate<IAttackTarget> oldValidator = innerValidator;
                    innerValidator = ((IAttackTarget t) => oldValidator(t) && MLB_AttackTargetFinder.CanReach(searcherThing, t.Thing, canBash));
                }
                bool flag = false;
                if (searcherThing.Faction != Faction.OfPlayer)
                {
                    //Log.Warning("Finder: Target available : " + ARA_AttackTargetFinder.tmpTargets.Count);
                    for (int i = 0; i < MLB_AttackTargetFinder.tmpTargets.Count; i++)
                    {
                        IAttackTarget attackTarget = MLB_AttackTargetFinder.tmpTargets[i];
                        if (attackTarget.Thing.Position.InHorDistOf(searcherThing.Position, maxDist) && innerValidator(attackTarget) && MLB_AttackTargetFinder.CanShootAtFromCurrentPosition(attackTarget, searcher, verb))
                        {
                ...
                if (flag)
                {
                    ...
                    MLB_AttackTargetFinder.tmpTargets.RemoveAll((IAttackTarget x) => !x.Thing.Position.InHorDistOf(searcherThing.Position, maxDist) || !innerValidator(x));
                    //Log.Warning("Finder: Target Avaliable : " + ARA_AttackTargetFinder.tmpTargets.Count);
                    result = MLB_AttackTargetFinder.GetRandomShootingTargetByScore(MLB_AttackTargetFinder.tmpTargets, searcher, verb);
                }
                else
                {
                    ...
                    if ((byte)(flags & TargetScanFlags.NeedReachableIfCantHitFromMyPos) != 0 && (byte)(flags & TargetScanFlags.NeedReachable) == 0)
                    {
                        ...
                        validator2 = ((Thing t) => innerValidator((IAttackTarget)t) && (MLB_AttackTargetFinder.CanReach(searcherThing, t, canBash) || MLB_AttackTargetFinder.CanShootAtFromCurrentPosition((IAttackTarget)t, searcher, verb)));
                    }
                    else
                    {
                    ...
                    result = (IAttackTarget)GenClosest.ClosestThing_Global(searcherThing.Position, MLB_AttackTargetFinder.tmpTargets, maxDist, validator2, null);
                }
                MLB_AttackTargetFinder.tmpTargets.Clear();
        ...
        private static IAttackTarget GetRandomShootingTargetByScore(List<IAttackTarget> targets, IAttackTargetSearcher searcher, Verb verb)
        {
            ...
            if (MLB_AttackTargetFinder.GetAvailableShootingTargetsByScore(targets, searcher, verb).TryRandomElementByWeight((Pair<IAttackTarget, float> x) => x.Second, out pair))
            {
                return pair.First;
            }
            return null;
        ...
        {
            MLB_AttackTargetFinder.availableShootingTargets.Clear();
            if (rawTargets.Count == 0)
            {
                return MLB_AttackTargetFinder.availableShootingTargets;
            }
            MLB_AttackTargetFinder.tmpTargetScores.Clear();
            MLB_AttackTargetFinder.tmpCanShootAtTarget.Clear();
            float num = 0f;
            IAttackTarget attackTarget = null;
            for (int i = 0; i < rawTargets.Count; i++)
            {
                MLB_AttackTargetFinder.tmpTargetScores.Add(-3.40282347E+38f);
                MLB_AttackTargetFinder.tmpCanShootAtTarget.Add(false);
                if (rawTargets[i] != searcher)
                {
                    bool flag = MLB_AttackTargetFinder.CanShootAtFromCurrentPosition(rawTargets[i], searcher, verb);
                    MLB_AttackTargetFinder.tmpCanShootAtTarget[i] = flag;
                    if (flag)
                    {
                        float shootingTargetScore = MLB_AttackTargetFinder.GetShootingTargetScore(rawTargets[i], searcher, verb);
                        MLB_AttackTargetFinder.tmpTargetScores[i] = shootingTargetScore;
            ...
            if (num < 1f)
            {
                ...
                {
                    MLB_AttackTargetFinder.availableShootingTargets.Add(new Pair<IAttackTarget, float>(attackTarget, 1f));
                }
            }
            else
            {
                ...
                for (int j = 0; j < rawTargets.Count; j++)
                {
                    ...
                    {
                        if (MLB_AttackTargetFinder.tmpCanShootAtTarget[j])
                        {
                            float num3 = MLB_AttackTargetFinder.tmpTargetScores[j];
                            if (num3 >= num2)
                            {
                                float second = Mathf.InverseLerp(num - 30f, num, num3);
                                MLB_AttackTargetFinder.availableShootingTargets.Add(new Pair<IAttackTarget, float>(rawTargets[j], second));
                            }
                        }
                    }
                }
        ...
        private static float GetShootingTargetScore(IAttackTarget target, IAttackTargetSearcher searcher, Verb verb)
        {
            ...
            return num + MLB_AttackTargetFinder.FriendlyFireShootingTargetScoreOffset(target, searcher, verb);
        }
        ...
        {
            return MLB_AttackTargetFinder.BestAttackTarget(searcher, flags, validator, minDistance, maxDistance, default(IntVec3), 3.40282347E+38f, false);
        }
        ...
        public static bool CanSee(this Thing seer, Thing target, Func<IntVec3, bool> validator = null)
        {
            ...
            ShootLeanUtility.LeanShootingSourcesFromTo(seer.Position, target.Position, seer.Map, MLB_AttackTargetFinder.tempSourceList);
            for (int j = 0; j < MLB_AttackTargetFinder.tempSourceList.Count; j++)
            {
                for (int k = 0; k < MLB_AttackTargetFinder.tempDestList.Count; k++)
                {
                    if (GenSight.LineOfSight(MLB_AttackTargetFinder.tempSourceList[j], MLB_AttackTargetFinder.tempDestList[k], seer.Map, true, validator, 0, 0))
                    {
                        return true;
                    }
                }
