...
namespace MuvLuvBeta.HarmonyInstance
{
    ...
    public static class MLB_JobGiver_Manhunter_TryGiveJob_RangedVerb_Patch
    {
        ...
        static bool Prefix(ref JobGiver_Manhunter __instance, ref Job __result, ref Pawn pawn)
        {
            ...
            Thing target = (Thing)MLB_AttackTargetFinder.BestAttackTarget((IAttackTargetSearcher)pawn, TargetScanFlags.NeedThreat | TargetScanFlags.NeedReachable, (Predicate<Thing>)(x =>
             x is Pawn || x is Building), 0.0f, 9999, new IntVec3(), float.MaxValue, false);

            //Seek thing hiding in embrasure.
            if (target == null)
                target = (Thing)MLB_AttackTargetFinder.BestAttackTarget((IAttackTargetSearcher)pawn, TargetScanFlags.NeedThreat, (Predicate<Thing>)(x =>
            x is Pawn || x is Building), 0.0f, 9999, new IntVec3(), float.MaxValue, false);
			...
			*/
            ...
            if (target == null)
            {
                shootable = (Thing)MLB_AttackTargetFinder.BestShootTargetFromCurrentPosition(pawn, (Predicate<Thing>)(x =>
