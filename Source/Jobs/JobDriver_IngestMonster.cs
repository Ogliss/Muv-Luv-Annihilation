...
{
	public class JobDriver_IngestMonster : JobDriver
	{
		...
		{
			Toil chew = Toils_MonsterIngest.ChewIngestible(pawn, ChewDurationMultiplier, TargetIndex.A, TargetIndex.B).FailOn((Toil x) => !IngestibleSource.Spawned && (pawn.carryTracker == null || pawn.carryTracker.CarriedThing != IngestibleSource)).FailOnCannotTouch(TargetIndex.A, PathEndMode.Touch);
		...
		private IEnumerable<Toil> PrepareToIngestToils_ToolUser(Toil chewToil)
		{
			...
			yield return Toils_MonsterIngest.PickupIngestible(TargetIndex.A, pawn);
			...
			if (!pawn.Drafted)
			{
				yield return Toils_MonsterIngest.CarryIngestibleToChewSpot(pawn, TargetIndex.A).FailOnDestroyedOrNull(TargetIndex.A);
			}
			yield return Toils_MonsterIngest.FindAdjacentEatSurface(TargetIndex.B, TargetIndex.A);
		}
		...
		private IEnumerable<Toil> TakeExtraIngestibles()
		{
			...
			Toil reserveExtraFoodToCollect = Toils_MonsterIngest.ReserveFoodFromStackForIngesting(TargetIndex.C);
