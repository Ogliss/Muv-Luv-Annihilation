...
{
    public static class Toils_MonsterIngest
    {
        ...
        public static IngestibleProperties GetIngestibleProperties(this Thing thing)
        {
            if (thing is Pawn pawn)
            {
                return pawn.RaceProps.corpseDef.ingestible;
            }
            return thing.def.ingestible;
        }
        ...
        public static Toil FinalizeIngest(Pawn ingester, TargetIndex ingestibleInd)
        {
            ...
            toil.initAction = delegate
            {
                ...
                {
                    victim.Kill(new DamageInfo(DamageDefOf.Bite, 9999, instigator: actor));
                    thing = victim.Corpse;
                }
                if (ingester.needs.mood != null && thing.def.IsNutritionGivingIngestible && thing.GetIngestibleProperties().chairSearchRadius > 10f)
                {
                    if (!(ingester.Position + ingester.Rotation.FacingCell).HasEatSurface(actor.Map) && ingester.GetPosture() == PawnPosture.Standing && !ingester.IsWildMan() && thing.GetIngestibleProperties().tableDesired)
                    {
